...
function Player:onLook(thing, position, distance)
	...
	-- Se for um item, verifica atributos especiais
	if thing:isItem() then
		local attributes = {
			...
			{name = "protect", label = "Protection"},
			{name = "lifesteal", label = "Life Steal"}
		}
		
		local hasAttribute = false
		for _, attr in ipairs(attributes) do
			local value = thing:getCustomAttribute(attr.name)
			if value then
				if not hasAttribute then
					description = description .. "\n\nAttributes:"
					hasAttribute = true
				end
				description = description .. string.format("\n[%s +%d]", attr.label, value)
			end
		end
	end
