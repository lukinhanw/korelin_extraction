...
for _, config in ipairs(spell.config) do
  ...
  function onGetFormulaValues(player, skill, attack, factor)
    local min = ((player:getLevel() + bender_attack['air_fan'].min) * bender_attack['air_fan'].min_mult) / 2 
    local max = ((player:getLevel() + bender_attack['air_fan'].max) * bender_attack['air_fan'].max_mult) / 2
    return -min, -max
  end
...
local function moveTargets(cid, dir)
    ...
    for i = 1, #targets[cid] do
      if isCreature(targets[cid][i]) then
  	    if isWalkable(getPositionByDirection(getThingPos(targets[cid][i]), dir, 1), true, true, true) then
          moveCreature(cid, targets[cid][i], dir)
  	    else
          targets[cid][i] = {}
        end
      end
...
function onCastSpell(player, var)
  ...
  targets[cid] = {}
  doSendAnimatedText("Fan", getThingPos(cid), TEXTCOLOR_AIR)  
  if player:canMove() == true then
    noMove(cid, 0.5, 3)
  end
  for n = 1, #spell.combats do
    addEvent(doCombat, (n * 120), cid, spell.combats[n], var)
    addEvent(moveTargets, n*120 + 5, cid, fanPush[getPlayerLookDir(cid)][n])
  end
  addEvent(cleanTargets, #spell.combats*120+5, cid)
  return true
end